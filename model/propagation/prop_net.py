...
class Decoder(nn.Module):
    ...
    def forward(self, f16, f8, f4):
        f16, f8, f4 = f16.half(), f8.half(), f4.half()
        self.compress.half()
        self.up_16_8.half()
        self.up_8_4.half()
        self.pred.half()
        ...
        return x.to("cpu").to(device=f16.device).float()
...
class EvalMemoryReader(nn.Module):
    ...
    def get_affinity(self, mk, qk):
        ...
        return affinity.to("cpu").to(device=mk.device)
    ...
    def readout(self, affinity, mv):
        ...
        return mem.to("cpu").to(device=mv.device)
...
class PropagationNetwork(nn.Module):
    ...
    def encode_value(self, frame, kf16, masks):
        ...
        print(frame.shape)
        print(f16.shape)
        return f16.unsqueeze(2).to("cpu").to(device=frame.device)  # B*512*T*H*W
