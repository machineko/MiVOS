...
class HyperParameters:
    ...
    def parse(self, unknown_arg_ok=False):
        ...
        # Data parameters
        parser.add_argument("--yv_root",
                            help="YouTubeVOS data root",
                            default="../YouTube")
        parser.add_argument("--davis_root",
                            help="DAVIS data root",
                            default="../DAVIS")
        ...
        parser.add_argument(
            "--steps",
            help="Iteration at which learning rate is decayed by gamma",
            default=None,
            type=int,
            nargs="*",
        )
        parser.add_argument("--gamma",
                            help="Gamma used in learning rate decay",
                            default=0.1,
                            type=float)

        # Loading
        parser.add_argument("--load_network",
                            help="Path to pretrained network weight only")
        parser.add_argument("--load_prop",
                            default="saves/propagation_model.pth")
        parser.add_argument(
            "--load_model",
            help=
            "Path to the model file, including network, optimizer and such",
        )
        ...
        parser.add_argument(
            "--id",
            help=
            "Experiment UNIQUE id, use NULL to disable logging to tensorboard",
            default="NULL",
        )
        parser.add_argument(
            "--debug",
        ...
        if self.args["stage"] == 0:
            self.args["iterations"] = none_or_default(self.args["iterations"],
                                                      30000)
